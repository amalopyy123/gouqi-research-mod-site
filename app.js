(() => {
  ...
  async function selectDataset(key) {
    state.key = key; state.query = ''; state.page = 1; state.learningType = ''; state.learningLevel = ''; els.search.value = ''; els.learningType.value = ''; els.learningLevel.value = ''; els.tableTitle.textContent = datasets[key].label; els.filters.hidden = !datasets[key].learning; showLoading();
    ...
    try { const data = await loadDataset(key); state.headers = data.headers; state.rows = data.rows; renderTable(); } catch (error) { els.tableState.hidden = false; els.tableState.textContent = `读取失败：${error.message}`; els.resultCount.textContent = ''; }
  }
  ...
  document.querySelectorAll('.tab').forEach(tab => tab.addEventListener('click', () => selectDataset(tab.dataset.dataset))); els.search.addEventListener('input', event => { state.query = event.target.value; state.page = 1; renderTable(); }); els.pageSize.addEventListener('change', event => { state.pageSize = Number(event.target.value); state.page = 1; renderTable(); }); els.learningType.addEventListener('change', event => { state.learningType = event.target.value; state.page = 1; renderTable(); }); els.learningLevel.addEventListener('change', event => { state.learningLevel = event.target.value; state.page = 1; renderTable(); });
  ...
  setupLinks(); selectDataset('variables'); window.requestAnimationFrame(syncTabScrollRange); window.setTimeout(syncTabScrollRange, 300);
})();
